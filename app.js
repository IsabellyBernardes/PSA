...
const upload = multer({
    ...
    limits: {
        fileSize: 5 * 1024 * 1024 // Limite de 5MB para o arquivo
    }
});

// Rota para processar o upload
app.post('/saveCarData', upload.single('carFile'), async (req, res) => {
    try {
        const carNumber = req.body.carNumber;
        const filePath = req.file.path;
        const userId = req.session.user_id; // Certifique-se de que o user_id está na sessão

        // Verifique se o user_id existe na sessão
        if (!userId) {
            return res.status(400).send('<p>Erro: Usuário não autenticado.</p>');
        }

        // Insira os dados no banco usando o Sequelize
        await sequelize.query(
            'INSERT INTO car_data (user_id, car_number, car_file, status) VALUES (:userId, :carNumber, :filePath, :status)', 
            {
                replacements: {
                    userId,
                    carNumber,
        ...
        res.redirect('/beneficiario-perfil');
    } catch (error) {
...
// Rota para salvar os dados do CAR
app.post('/saveCarData', upload.single('carFile'), async (req, res) => {
    const { carNumber } = req.body;
    const carFile = req.file ? req.file.filename : null; // Pega o nome do arquivo carregado

    try {
        // Salvar no banco de dados
        await pool.query('INSERT INTO car_data (car_number, car_file) VALUES ($1, $2)', [carNumber, carFile]);
        res.send('Dados do CAR salvos com sucesso!');
    } catch (err) {
        console.error('Erro ao salvar dados do CAR:', err);
...
// Servir o arquivo de upload (necessário para os arquivos enviados)
